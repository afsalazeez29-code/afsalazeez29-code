...
svg=[f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" role="img" aria-labelledby="title desc"><title id="title">{data["username"]} contribution graph</title><desc id="desc">Animated 53-week GitHub-style contribution heatmap with real public activity data.</desc><style>.c{{opacity:0;animation:appear .30s ease-out forwards}}@keyframes appear{{to{{opacity:1}}}}text{{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}}</style>',f'<rect width="{w}" height="{h}" rx="12" fill="#0d1117"/><rect x=".5" y=".5" width="{w-1}" height="{h-1}" rx="12" fill="none" stroke="#30363d"/><text x="20" y="25" fill="#c9d1d9" font-size="13">{data["total_contributions"]:,} contributions in the last year</text>']
seen=set()
...
for col in range(cols):
  for row in range(rows):
    day=start+dt.timedelta(days=col*7+row); item=days.get(day.isoformat()); level=item['level'] if item else 0; count=item['count'] if item else 0; delay=(col*.0216+row*.042)
